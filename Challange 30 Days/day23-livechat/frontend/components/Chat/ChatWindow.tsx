...
export default function ChatWindow() {
    ...
    useEffect(() => {
        if (activeConversationId) {
            setMessages([
                { id: 1, content: "هلا والله! كيف الحال؟", sender_id: activeConversation?.participants[0]?.user_id || 0, created_at: new Date().toISOString() },
                { id: 2, content: "الحمدلله بخير، جالس أجرب التطبيق الجديد 🔥", sender_id: user?.id, created_at: new Date().toISOString() },
                { id: 3, content: "ما شاء الله، التصميم رهيب!", sender_id: activeConversation?.participants[0]?.user_id || 0, created_at: new Date().toISOString() },
            ]);
        }
    ...
    if (!activeConversation) return <div className="h-full flex items-center justify-center text-slate-400">اختر محادثة عشان تبدأ</div>;
